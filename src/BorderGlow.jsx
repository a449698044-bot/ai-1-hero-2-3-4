...
function buildGradientVars(colors) {
  return GRADIENT_KEYS.reduce((vars, key, index) => {
    vars[key] = `radial-gradient(at ${GRADIENT_POSITIONS[index]}, ${colors[Math.min(COLOR_MAP[index], colors.length - 1)]} 0px, transparent 50%)`;
    if (index === GRADIENT_KEYS.length - 1) vars['--gradient-base'] = `linear-gradient(${colors[0]} 0 100%)`;
    return vars;
  }, {});
}
